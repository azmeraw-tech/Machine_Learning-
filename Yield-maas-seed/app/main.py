...
async def startup_event() -> None:
    try:
        logger.info("Loading latest model artefacts…")
        models = model_loader.load_latest_model()  # Assume this returns a dict of 3 models

        # Example: {'model_soil': model1, 'model_crop': model2, 'model_fert': model3}
        for key, model in models.items():
            redis_cache.cache_model(model, key)
            logger.info(f"{key} cached successfully.")

        # Verify caching
        for key in models.keys():
            cached = redis_cache.get_cached_model(key)
            if cached is None:
                logger.error(f"{key} was not cached properly!")
            else:
                logger.info(f"{key} caching verified successfully.")
