...
def predict_factors(models, year, base_year=2023):
    steps = int(year- base_year)
    # List to store forecasts for 2024
    forecast_next = []
    for factor, model_info in models.items():
        arima_model = model_info[f'{factor}_model']

        # Predict next year (2024)
        forecast_value = np.exp(arima_model.forecast(steps=steps))  # Apply inverse log transformation
        forecast_next.append(list(forecast_value.loc[:]))  # Append the first value of the forecast
...
# # ────────────────────────────────────────────────────────────────
def run_inference(model: object,encoder:object,lonlat_mapper:object,Yield:object,payload_from_client: Dict, kdtree: KDTree, ficos,loaded_models,factor_model) -> Dict:
...
#Yield
    ...
    crop_encoded = map_value(crop_type_mapping, crop_type, "Crop type")
    ...
    normalized_yield = normalize_yield(crop_type, predicted_yield)
...
#NDVI
    ...
    try:
        ...
        # fico_price = predict_price(crop_type, year, predicted_yield)
...
#Soil
        ...
        if crop_type not in crop_similarity_lower:
            return "You asked for a product that is out of range."

        similarity_score = crop_similarity_lower.get(crop_type, {}).get(predicted_crop.lower(), 0)
        ...
        if crop_type not in loaded_models:
            raise ValueError(f"No model found for item: {crop_type}")

        model_info = loaded_models[crop_type]
        model = model_info['model']
        scaler = model_info['scaler']
        columns = model_info['columns']
        previous_year_price = model_info['previous_year_price']
        base_year=model_info['base_year']
        steps =  year - base_year
        #Predict factors
        factors = np.array(predict_factors(factor_model, year,base_year))
        predicted_prices=[previous_year_price]
        for i in range(steps):
            factor_i =  list(factors[:,i])
            factor_i.append(predicted_prices[-1])
            # Ensure input has valid factor names
            factor_df = pd.DataFrame([factor_i], columns=columns)
            scaled_factors = scaler.transform(factor_df)
            pre_p = model.predict(scaled_factors)
            predicted_prices.append(pre_p[0])

        predicted_price = predicted_prices[-1]

        p_stats = crop_price_stats.get(crop_type)
        y_stats = crop_yield_stats.get(crop_type)
        total_min_price = y_stats["min"]*p_stats["min"]
        total_max_price = y_stats["max"]*p_stats["max"]
        pYield = predicted_yield
        total_predicted_amount = pYield*predicted_price

        price_score = 300 + (550*(total_predicted_amount-total_min_price)/(total_min_price+total_max_price))
        price_score = round(max(300, min(850, price_score)),2)
